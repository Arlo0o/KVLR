...
from KVLR.utils.config import parse_configs
from KVLR.utils.logger import create_logger
from KVLR.utils.misc import format_duration
from KVLR.utils.train import setup_device
...
def get_seed_worker(seed):
    return functools.partial(_seed_worker, seed)
...
def prepare_dataloader(
    ...
    **kwargs,
    ...
    if isinstance(dataset, VideoTextDataset) and bucket_config is not None:
...
def collate_fn_default(batch):
    ...
    # skeleton_maps is only present for samples that have a matching PTH file.
    # If any sample in the batch is missing it, drop it from all samples so that
    # default_collate sees a homogeneous batch.  The training loop already handles
    # the case where skeleton_maps is absent (batch.pop("skeleton_maps", None)).
    if not all("skeleton_maps" in x for x in batch):
        for x in batch:
            x.pop("skeleton_maps", None)
...
if __name__ == "__main__":
    ...
    # Example usage:
    # torchrun --nproc_per_node 1 -m KVLR.datasets.dataloader configs/diffusion/train/video_cond.py
    cfg = parse_configs()
    setup_device()
