...
def is_valid_video_file(path: str, num_probe_frames: int = 1) -> bool:
    """Lightweight validation to detect obviously corrupted videos before decoding.

    Args:
        path (str): Path to the video file.
        num_probe_frames (int): Number of frames to probe. Defaults to 1 to minimize I/O.

    Returns:
        bool: True if the video looks readable, False if it is missing or cannot be opened.
    """

    if not os.path.exists(path):
        return False

    # When OpenCV is unavailable we can only rely on existence checks.
    if cv2 is None:
        return True
    ...
    ok = True
    for _ in range(max(1, num_probe_frames)):
        ret, _ = cap.read()
        if not ret:
            ok = False
            break
