...
from KVLR.datasets import save_sample
from KVLR.datasets.aspect import get_image_size
from KVLR.datasets.utils import read_from_path, rescale_image_by_path
from KVLR.utils.logger import log_message
from KVLR.utils.prompt_refine import refine_prompts


class SamplingMethod(Enum):
    I2V = "i2v"  # for KVLR image-to-video generation
...
def create_tmp_csv(save_dir: str, prompt: str, ref: str = None, create=True) -> str:
    ...
    with open(tmp_file, "w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        if ref is not None:
            writer.writerow(["text", "ref"])
            writer.writerow([prompt, ref])
        else:
