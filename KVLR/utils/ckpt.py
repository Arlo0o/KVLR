...
def master_weights_gathering(model: torch.nn.Module, optimizer: LowLevelZeroOptimizer, pinned_state_dict: dict) -> None:
    ...
    for name, param in model.named_parameters():
        if id(param) not in w2m:
            # Frozen parameter not managed by the optimizer — save its current value directly
            if dist.get_rank() == 0:
                pinned_state_dict[name].copy_(param.data.cpu())
            continue
...
def load_master_weights(model: torch.nn.Module, optimizer: LowLevelZeroOptimizer, state_dict: dict) -> None:
    ...
    for name, param in model.named_parameters():
        if id(param) not in w2m:
            # Frozen parameter — skip (it will be loaded via model state_dict)
            continue
        master_p = w2m[id(param)]
        ...
        if self.master_pinned_state_dict is None and dist.get_rank() == 0:
            ...
            for n, p in model.named_parameters():
                if id(p) in w2m:
                    ...
                    # Frozen parameter — use its own dtype
                    sd[n] = torch.empty(p.shape, dtype=p.dtype, pin_memory=True, device="cpu")
