...
def mmdit_model_forward(
    ...
    **kwargs,
    ...
    if shard_config.pipeline_stage_manager is None or shard_config.pipeline_stage_manager.is_first_stage():
        ...
        if shard_config.enable_sequence_parallelism:
            ...
            if 0 in img_splits:
                ...
                if dist.get_rank(shard_config.sequence_parallel_process_group) == 0:
                    print(
                        f"[SP fallback] disable sequence parallelism for this forward: "
                        f"txt_splits={txt_splits}, img_splits={img_splits}"
                    )
        ...
        action_encoded = None
        skeleton_maps = kwargs.get("skeleton_maps", None)
        ...
        # print(f"========= [mmdit_model_forward(Distributed)] use_action: {getattr(self, 'use_action_condition', False)}, has_skmaps: {has_skmap}, img_shape: {img_shape}, keys: {skmap_keys} =========")
        
        if getattr(self, "use_action_condition", False) and skeleton_maps is not None and img_shape is not None:
            action_encoded = self._encode_skeleton_maps(skeleton_maps, img_shape, timesteps)
            if shard_config.enable_sequence_parallelism and "img_splits" in locals() and 0 not in img_splits:
                action_encoded = split_forward_gather_backward_var_len(
                    action_encoded, 1, shard_config.sequence_parallel_process_group, img_splits
                )
    else:
        img, txt, vec, pe = internal_img, internal_txt, internal_vec, internal_pe
        action_encoded = kwargs.get("internal_action_encoded", None)
    ...
    proj_d_idx = 0
    if getattr(self, "use_action_condition", False) and getattr(self, "_action_injection_double_layers", None) is not None:
        for i in range(double_start):
            if i in self._action_injection_double_layers:
                proj_d_idx += 1

    for i, block in enumerate(self.double_blocks[double_start:double_end], start=double_start):
        img, txt = auto_grad_checkpoint(block, img, txt, vec, pe)
        if action_encoded is not None and getattr(self, "_action_injection_double_layers", None) is not None:
            if i in self._action_injection_double_layers:
                img = img + self.action_proj_double[proj_d_idx](action_encoded)
                proj_d_idx += 1
    ...
    if shard_config.pipeline_stage_manager is not None and stage_index[1] <= len(self.double_blocks):
        return {
            ...
            "internal_pe": pe,
            "internal_vec": vec,
            "internal_action_encoded": action_encoded,
        }
    ...
    proj_s_idx = 0
    if getattr(self, "use_action_condition", False) and getattr(self, "_action_injection_single_layers", None) is not None:
        for i in range(single_start):
            if i in self._action_injection_single_layers:
                proj_s_idx += 1

    if shard_config.enable_sequence_parallelism and "txt_splits" in locals():
        local_txt_len = txt_splits[dist.get_rank(shard_config.sequence_parallel_process_group)]
    else:
        local_txt_len = txt_len

    for i, block in enumerate(self.single_blocks[single_start:single_end], start=single_start):
        img = auto_grad_checkpoint(block, img, vec, pe)
        if action_encoded is not None and getattr(self, "_action_injection_single_layers", None) is not None:
            if i in self._action_injection_single_layers:
                # img_part = img[:, txt_len:, :] + self.action_proj_single[proj_s_idx](action_encoded)
                # img = torch.cat([img[:, :txt_len, :], img_part], dim=1)
                img_part = img[:, local_txt_len:, :] + self.action_proj_single[proj_s_idx](action_encoded)
                img = torch.cat([img[:, :local_txt_len, :], img_part], dim=1)
                proj_s_idx += 1
